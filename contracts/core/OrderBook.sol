...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    function getUsdgMinPrice(address _otherToken)
        public
        view
        returns (uint256)
    {
    ...
    function executeIncreaseOrder(
        ...
        IBasePositionManager positionManager_ = IBasePositionManager(
            positionManager
        );
        if (order.isLong) {
            uint256 maxGlobalLongSize = positionManager_.maxGlobalLongSizes(
                order.indexToken
            );
            if (
                maxGlobalLongSize > 0 &&
                IVault(vault).guaranteedUsd(order.indexToken).add(
                    order.sizeDelta
                ) >
                maxGlobalLongSize
            ) {
            ...
            uint256 maxGlobalShortSize = positionManager_.maxGlobalShortSizes(
                order.indexToken
            );
            if (
                maxGlobalShortSize > 0 &&
                IVault(vault).globalShortSizes(order.indexToken).add(
                    order.sizeDelta
                ) >
                maxGlobalShortSize
            ) {
